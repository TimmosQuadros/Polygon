...
CREATE TABLE IF NOT EXISTS `polygon`.`floorplans` (
  ...
  INDEX `fk_floorplans_buildings1_idx` (`building_id` ASC, `organisations_id` ASC),
  ...
  CONSTRAINT `fk_floorplans_buildings1`
    FOREIGN KEY (`building_id` , `organisations_id`)
    REFERENCES `polygon`.`buildings` (`building_id` , `organisations_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
AUTO_INCREMENT = 3
